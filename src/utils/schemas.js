...
const cardDocsSchema = Joi.array().items(
  Joi.object({
...
const revsObjectSchema = Joi.object().pattern(
  /^card-[0-9a-f]{8}-(?:[0-9a-f]{4}-){3}[0-9a-f]{12}$/,
  Joi.array().items(Joi.string().pattern(/^\d+-[a-f0-9]+$/))
)

module.exports = { cardDocsSchema, revsObjectSchema }
